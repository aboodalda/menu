...
function renderLedger(){
  ...
  if (allOrders.length === 0) {
    body.innerHTML = '<tr><td colspan="4">ما في طلبات لسه</td></tr>';
    grandTotalEl.textContent = `0 ${CURRENCY}`;
    return;
  }

  // ترتيب تصاعدي حسب رقم الطلب (١ فصاعدًا)
  const sorted = [...allOrders].sort((a, b) => (a.number || 0) - (b.number || 0));
...
function recalcGrandTotal(){
  const grandTotalEl = document.getElementById('ledgerGrandTotal');
  let sum = 0;
  document.querySelectorAll('.ledger-price-input').forEach(inp => {
