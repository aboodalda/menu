...
const CURRENCY = "₪";
// رقم واتساب الكافتيريا — بصيغة دولية وبدون + أو 00 (مثال: فلسطين 970 + الرقم بدون الصفر الأول)
const CAFE_WHATSAPP = "970599000000";
const CATEGORIES = [
...
/* ---------- ضغط وتصغير الصور تلقائيًا (بدون رفع أي صورة من جديد) ---------- */
// بتمرّر أي رابط صورة عبر خدمة wsrv.nl المجانية، وبترجع نسخة مصغّرة ومضغوطة
// بصيغة WebP خفيفة — بتحل مشكلة بطء التحميل حتى لو الصورة الأصلية كبيرة جدًا.
function optimizeImg(url, width){
  if (!url) return url;
  const params = new URLSearchParams({
    url, w: String(width), q: '75', output: 'webp', default: url,
  });
...
/* ---------- كاروسيل صور الهيرو (يعرض صور الأصناف الحقيقية تلقائيًا) ---------- */
let heroTimer = null;
...
function buildHeroCarousel(){
  ...
  // ناخذ حتى 5 صور مختلفة من الأصناف الموجودة فعليًا
  const seen = new Set();
  const images = [];
  allItems.forEach(it => {
    if (it.img && !seen.has(it.img) && images.length < 2) {
      seen.add(it.img);
      images.push(it.img);
    }
  });
  ...
  images.forEach((src, i) => {
    ...
    img.src = optimizeImg(src, 900);
    img.alt = '';
    img.loading = i === 0 ? 'eager' : 'lazy';
    if (i === 0) img.classList.add('active');
    carousel.insertBefore(img, document.getElementById('heroDots'));
  });

  const dotsEl = document.getElementById('heroDots');
  dotsEl.innerHTML = images.length > 1
    ? images.map((_, i) => `<button data-i="${i}" class="${i===0?'active':''}"></button>`).join('')
    : '';

  heroSlideIndex = 0;
  goToHeroSlide(0, carousel);

  dotsEl.querySelectorAll('button').forEach(b => {
    b.onclick = () => goToHeroSlide(parseInt(b.dataset.i), carousel);
  });

  if (images.length > 1) {
    heroTimer = setInterval(() => {
      goToHeroSlide((heroSlideIndex + 1) % images.length, carousel);
    }, 3000);
  }
}

function goToHeroSlide(index, carousel){
...
/* ---------- القائمة المنسدلة للأقسام (بالشريط العلوي) ---------- */
const dropdownBtn = document.getElementById('menuDropdownBtn');
const dropdownPanel = document.getElementById('dropdownCategories');

function populateDropdown(){
  dropdownPanel.innerHTML = CATEGORIES.map(c => `<button data-cat="${c.id}">${c.name}</button>`).join('');
  dropdownPanel.querySelectorAll('button').forEach(b => {
    b.onclick = () => {
      const section = document.getElementById(b.dataset.cat);
...
/* ---------- تحديث القسم النشط تلقائيًا أثناء التمرير (Scroll Spy) ---------- */
let sectionToButton = {};
...
function setupScrollSpy(){
  ...
  const navHeight = tabsEl.offsetHeight || 60;
  scrollSpyObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (!entry.isIntersecting) return;
      const btn = sectionToButton[entry.target.id];
      if (!btn) return;
      [...tabsEl.children].forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      btn.scrollIntoView({ behavior:'smooth', inline:'center', block:'nearest' });
    });
  }, { rootMargin: `-${navHeight + 10}px 0px -65% 0px`, threshold: 0 });

  document.querySelectorAll('section.category').forEach(sec => scrollSpyObserver.observe(sec));
}
...
function renderItemCard(it){
  ...
  const media = it.img
    ? `<img src="${optimizeImg(it.img, 500)}" alt="${it.name}" loading="lazy">`
...
document.getElementById('submitOrderBtn').onclick = async () => {
  ...
  try {
    ...
    document.getElementById('trackOrderBtn').href = `track.html?id=${docRef.id}`;
    const waText = encodeURIComponent(`مرحبا، طلبي رقم #${orderNumber} — بدي أتابعه`);
    document.getElementById('whatsappBtn').href = `https://wa.me/${CAFE_WHATSAPP}?text=${waText}`;
